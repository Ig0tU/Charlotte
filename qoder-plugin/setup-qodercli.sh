...
if [ $# -ge 1 ]; then
    ...
    # Default to cli-anything-web-plugin relative to qoder-plugin directory
    if ! PLUGIN_DIR="$(cd "${SCRIPT_DIR}/../cli-anything-web-plugin" 2>/dev/null && pwd)"; then
        echo -e "${YELLOW}Error: Could not find cli-anything-web-plugin directory at ${SCRIPT_DIR}/../cli-anything-web-plugin${NC}" >&2
        echo -e "${YELLOW}Please provide an explicit plugin path:${NC}" >&2
        echo -e "${YELLOW}  bash setup-qodercli.sh /path/to/cli-anything-web-plugin${NC}" >&2
        exit 1
    fi
fi
